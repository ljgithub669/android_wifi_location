...
/////////////////////////////////////////////////////////////////////////////////////////////////////////// 
//用户配置区
//extern u8 zuizhong[USART3_MAX_RECV_LEN][USART3_MAX_RECV_LEN];
//const u8 zuizhong[USART3_MAX_RECV_LEN][USART3_MAX_RECV_LEN];
//连接端口号:8086,可自行修改为其他端口.
const u8* portnum="8080";		 

//WIFI STA模式,设置要去连接的路由器无线参数,请根据你自己的路由器设置,自行修改.
const u8* wifista_ssid="estrella";			//路由器SSID号
const u8* wifista_encryption="wpawpa2_aes";	//wpa/wpa2 aes加密方式
const u8* wifista_password="11111111"; 	//连接密码
...
void mpu6050(void)
{
	...
	{	
			...
			if(Task_Delay[1]==0)
			{
				...
				//xyz轴的加速度,收到后要先除以2096，之后乘9.8，单位是米每二次方秒
				MPU6050ReadGyro(Gyro);
				printf("    陀螺仪%8d%8d%8d",Gyro[0],Gyro[1],Gyro[2]);   //角速度
...
{
	int i,j,k;
		u8 zuizhong[USART3_MAX_RECV_LEN]="0";
	...
	//	printf("%s",USART3_RX_BUF);	//发送到串口
		...
		{
		    if(USART3_RX_BUF[i+1]=='C'&&USART3_RX_BUF[i+2]=='W'&&USART3_RX_BUF[i+3]=='L'&&USART3_RX_BUF[i+4]=='A')
				{
				    i=i+11;
				  for(k=0;k<100;k++)
          {
					   if(USART3_RX_BUF[i+k]=='\"')
							 break;
					}
				//	u8 zuizhong[USART3_MAX_RECV_LEN]="0";
					for(j=0;j<k;j++)
					{
	           zuizhong[j]=USART3_RX_BUF[i+j];
					}
					    zuizhong[k]=' ';
					    i=i+k;
					zuizhong[k+1]=USART3_RX_BUF[i+1];    //这个是负号
					zuizhong[k+2]=USART3_RX_BUF[i+2];
					zuizhong[k+3]=USART3_RX_BUF[i+3];
					 if(USART3_RX_BUF[i+4]>='0'&&USART3_RX_BUF[i+4]<='9'){
                 zuizhong[k+4]=USART3_RX_BUF[i+4] ;
	               zuizhong[k+5]=' ';
           	}else {zuizhong[k+4]=' ';zuizhong[k+5]=' ';}
					zuizhong[k+6]=USART3_RX_BUF[i+25];
	          if(USART3_RX_BUF[i+26]>='0'&&USART3_RX_BUF[i+26]<='9')
          	{
	          zuizhong[k+7]=USART3_RX_BUF[i+26];
	             zuizhong[k+8]='\0';
	          }else  { zuizhong[k+7]=' ';zuizhong[k+8]='\0';}
...
//void  fanhui(u8 USART3_RX_BUF[USART3_MAX_RECV_LEN])
//{  
//    u16 i;
//	static u8 k=0;
//	u8 zuizhong[USART3_MAX_RECV_LEN][USART3_MAX_RECV_LEN]="0";
//	zuizhong[k][0]=USART3_RX_BUF[11];
//	zuizhong[k][1]=USART3_RX_BUF[12];
//	zuizhong[k][2]=USART3_RX_BUF[13];
//	zuizhong[k][3]=' ';
//	zuizhong[k][4]=USART3_RX_BUF[17];
//	zuizhong[k][5]=USART3_RX_BUF[18];
//	if(USART3_RX_BUF[19]>='0'&&USART3_RX_BUF[19]<='9'){
//     zuizhong[k][6]=USART3_RX_BUF[19] ;
//	}else zuizhong[k][6]=' ';
//	 zuizhong[k][7]=USART3_RX_BUF[40];
//	if(USART3_RX_BUF[41]>='0'&&USART3_RX_BUF[19]<='9'){
//		zuizhong[k][8]=USART3_RX_BUF[41];
//	zuizhong[k][9]='\0';
//	}
//	else zuizhong[k][8]='\0';
//	printf("%s ",zuizhong[k]);
...
{  
    u16 i;
	u8 zuizhong[USART3_MAX_RECV_LEN]="0";
	zuizhong[0]=USART3_RX_BUF[11];
	zuizhong[1]=USART3_RX_BUF[12];
	zuizhong[2]=USART3_RX_BUF[13];
	zuizhong[3]=' ';
	zuizhong[4]=USART3_RX_BUF[17];
	zuizhong[5]=USART3_RX_BUF[18];
	if(USART3_RX_BUF[19]>='0'&&USART3_RX_BUF[19]<='9'){
     zuizhong[6]=USART3_RX_BUF[19] ;
	    zuizhong[7]=' ';
	}else {zuizhong[6]=' ';zuizhong[7]=' ';}
	zuizhong[8]=USART3_RX_BUF[40];
	if(USART3_RX_BUF[41]>='0'&&USART3_RX_BUF[41]<='9')
	{
	   zuizhong[9]=USART3_RX_BUF[41];
	   zuizhong[10]='\0';
	}else  { zuizhong[9]=' ';;zuizhong[10]='\0';}
	printf("%s ",zuizhong);
	//RTC_Init();
	RTC_Get();
